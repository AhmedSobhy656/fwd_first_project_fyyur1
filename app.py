...
from forms import *
from flask_migrate import Migrate # added as
...
db = SQLAlchemy(app)
migrate = Migrate(app, db)  #added as 
...
def venues():
  ...
  date_now = datetime.now()       #--- added as ---#
  venues = Venue.query.group_by(Venue.id, Venue.state, Venue.city).all()      #--- added as ---#
  venue_city_state = ''      #--- added as ---#
  data=[{
    "city": "San Francisco",
    "state": "CA",
    "venues": [{
  ...
  for venue in venues:
    ...
    # update_shows_data : show data update 
    update_shows_data = Show.query.filter(Show.start_time > date_now).all()
    if venue_city_state == venue.city + venue.state:
      data[len(data) - 1]["venues"].append({
        "id": venue.id,
        "name":venue.name,
        "num_upcoming_shows": len(update_shows_data)
      })
    else:
...
def search_venues():
  ...
  list_of_artists = list(map(Artist.less, artists))
  search_list = list(map(Venue.less, venue)) 

  response = {
    "count":len(search_list),
    "data": [search_list, list_of_artists] #Kindly Check {search_venues.html}
  }
  print(artists)
...
def show_venue(venue_id):
  ...
  # TODO: replace with real venue data from the venues table, using venue_id
   # --- added as --
  new_instance_venue_added = Venue()
  data4_added = new_instance_venue_added.venue_all_data(venue_id) #Did this just to keep the mock data as well
   # --- added as --#
  data = list(filter(lambda d: d['id'] == venue_id, [data1, data2, data3, data4_added]))[0]
...
def artists():
  ...
  all_artists = Artist.query.all() #--added as ---#
  data=[{
    "id": 4,
    "name": "Guns N Petals",
  }, {
    ...
    #--added as
  my_list= list(map(Artist.less, all_artists))   
  for x in range(len(my_list)):
    data.append(my_list[x])  #--added as ---#
...
def shows():
  ...
  shows_all = Show.query.all()                # --- added  as --- #
